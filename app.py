...
def run_bot():
    while True:
        try:
            ...
            # 2️⃣ Fetch balance once for equity tracking
            ...
            # 5️⃣ Skip if not enough ATR or EMA slope
            if state.position is None:
                ...
                if exit_price:
                    ...
                    fee = abs(state.size*state.entry_price)*FEE_RATE + abs(state.size*exit_price)*FEE_RATE
                    pnl -= fee
                    state.capital += pnl
